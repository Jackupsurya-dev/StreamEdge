...
func (r *Redis) SaveUser(id int, user interface{}) error {
	...
	if err != nil {
		logger.Log.Errorln("Failed to marshal user to JSON:", err)
		return err
	}
	...
	if err != nil {
		logger.Log.Errorf("Failed to save user in Redis with key %s: %v\n", key, err)
		return err
	}

	logger.Log.Infof("Successfully saved user in Redis with key: %s\n", key)
	return nil
}
