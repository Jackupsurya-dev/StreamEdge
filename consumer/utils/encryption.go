...
func Encrypt(data, key string) (string, error) {
	...
	if err != nil {
		logger.Log.Errorln("Failed to create new cipher:", err)
		return "", err
	}
	...
	if _, err := io.ReadFull(rand.Reader, iv); err != nil {
		logger.Log.Errorln("Failed to generate random IV:", err)
		return "", err
	}
...
func Decrypt(encryptedData, key string) (string, error) {
	...
	if err != nil {
		logger.Log.Errorln("Failed to decode base64:", err)
		return "", err
	}
	...
	if len(cipherText) < aes.BlockSize {
		logger.Log.Errorln("Ciphertext too short")
	...
	if err != nil {
		logger.Log.Errorln("Failed to unpad plaintext:", err)
		return "", err
	}
...
func PKCS7Unpadding(data []byte, blockSize int) ([]byte, error) {
	...
	if length == 0 || length%blockSize != 0 {
		logger.Log.Errorln("Invalid padding size")
		return nil, errors.New("invalid padding size")
	}

	padding := int(data[length-1])
	if padding > length || padding > blockSize {
		logger.Log.Errorln("Invalid padding")
