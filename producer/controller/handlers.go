...
func UploadCSV(rabbitMQProducer *rabbitmq.Producer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Log.Errorln("Failed to parse form:", err)
		...
		if err != nil {
			logger.Log.Errorln("Failed to retrieve the file from form:", err)
		...
		if err != nil {
			logger.Log.Errorln("Failed to save the file:", err)
		...
		if err != nil {
			logger.Log.Errorln("Failed to save the file content:", err)
		...
		if err != nil {
			logger.Log.Errorln("Failed to read the uploaded file:", err)
		...
		if err != nil {
			logger.Log.Errorln("Failed to parse CSV file:", err)
		...
		if len(records) < 2 {
			logger.Log.Errorln("CSV file must have a header and at least one row")
		...
		go func() {
			...
			for i, row := range records[1:] {
				if len(row) != len(headers) {
					logger.Log.Errorln("Row", i+2, "does not match the header length")
				...
				if err != nil {
					logger.Log.Infof("Invalid ID at row %d: %v\n", i+2, err)
					continue
				}
				...
				if err != nil {
					logger.Log.Infof("Failed to marshal row %d to JSON: %v\n", i+2, err)
					continue
				}

				// Encrypt user data
				encryptionKey := viper.GetString("encryption_key")
				encryptedData, err := utils.Encrypt(string(userJSON), encryptionKey)
				if err != nil {
					logger.Log.Infof("Failed to encrypt user data: %v\n", err)
					continue
				}
				...
				if err != nil {
					logger.Log.Infof("Failed to publish row %d to RabbitMQ: %v\n", i+2, err)
					continue
				}
			}
		}()
