...
func (p *Postgres) GetFilteredUsers(filters map[string][]string) ([]map[string]interface{}, error) {
	...
	for key, value := range filters {
		if !isValidColumnName(key) {
			logger.Log.Errorf("invalid filter key: %s\n", key)
			return nil, fmt.Errorf("invalid filter key: %s", key)
		}
		if len(value) != 1 {
			logger.Log.Errorf("multiple values are not supported for filter key: %s\n", key)
	...
	if err != nil {
		logger.Log.Errorln("Failed to execute query:", err)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(valuePtrs...); err != nil {
			logger.Log.Errorln("Failed to scan row: ", err)
			return nil, err
		}
	...
	if err := rows.Err(); err != nil {
		logger.Log.Errorln("Failed to retrieve rows:", err)
		return nil, err
	}
